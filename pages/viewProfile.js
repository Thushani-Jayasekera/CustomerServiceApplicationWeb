...
import { Block, Box, Button, Columns, Container, Content, Level, Section,Message } from "react-bulma-components";
import { useQuery } from "@apollo/client";
import { GET_ME, GET_REVIEWED_REQ_OF_PROVIDER, GET_USER_BY_ID, GET_USER_WITH_REVIEWS } from "../gql/query";
    ...
    <Container>
        ...
        <Section>
          <Columns>
            <Columns.Column>
              <Level>
                <Level.Item>
                  <img crossOrigin={"anonymous"} src={data.getUserbyId.profile_url || profileImg} width={"256px"} />
                </Level.Item>
                <Level.Item className="mx-4">
                    <p className={"has-text-link"}>"{data.getUserbyId.bio}"</p>
                  </Level.Item>
      ...
      {requests.length !== 0 ? (
        ...
        <Message color={'danger'}>
          <Message.Body>No Reviewed Requests! Check Later</Message.Body>
        </Message>
      )}
    </>
                </Level.Item>
            ...
            <Columns.Column>
              <Level>
                <Level.Side>
                  ...
                  <Level.Item>
                    <h6>  <MapPin/> {data.getUserbyId.town}  {data.getUserbyId.city}</h6>
                  </Level.Item>
              </Level>
              <Level>
                <Level.Side>
              ...
              <Level>
                ...
                <Level.Side align={"right"}>
                  <Level.Item>
                    {
                      (data.getUserbyId.provider_rating && data.getUserbyId.provider_rating > 0) && (
                        <ReactStars edit={false} value={data.getUserbyId.provider_rating} count={5} size={36} />
                      )
                    }
